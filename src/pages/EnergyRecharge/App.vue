...
export default defineComponent({
  ...
  setup() {
    ...
    Promise.all(
      Object.keys(CHARACTER_MASTER).map((s) =>
        getCharacterMasterDetail(s as TCharacterKey)
      )
    ).then((rets) => {
      rets.forEach((ret) => {
    ...
    const characterImgClass = (item: TCharacterDetail) =>
      STAR_BACKGROUND_IMAGE_CLASS[item.レアリティ];

    const weaponTypeList = reactive(Object.keys(WEAPON_MASTER) as TWeaponTypeKey[]);
    const favoniusWeaponMap = computed(
      (): Map<TWeaponTypeKey, TWeaponEntry> => {
        const result = new Map();
        for (const type of weaponTypeList) {
          const weapon = Object.keys(WEAPON_MASTER[type]).filter((s) =>
            s.startsWith("西風")
          )[0];
