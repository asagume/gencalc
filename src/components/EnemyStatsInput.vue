...
export default defineComponent({
    name: 'EnemyStatsInput',
    props: {
        visible: Boolean,
        lang: String,
        enemyStatsObj: Object,
    },
    emits: ['update:stat-adjustment'],
    setup(props) {
        const displayName = (name: string | number) => name;
        const displayStatValue = (stat: string) => {
            if (props.enemyStatsObj && props.enemyStatsObj[stat]) {
                return props.enemyStatsObj[stat];
            }
            return 0;
        };
        ...
        const enemyStatsCategory = ['元素ステータス·耐性'];
        const categoryList = enemyStatsCategory;
        ...
        const visibleStatList = (category: string) => statList(category)?.filter(stat => categoryOpenClose.value[category] || (props.enemyStatsObj && props.enemyStatsObj[stat])) ?? [];
