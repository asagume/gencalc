...
function calculateDamageFromDetail(statusObj, detailObj, opt_element = null) {
    ...
    天賦性能変更系詳細ArrMapVar.forEach((value, key) => {
        value.forEach(valueObj => {
            ...
            if (valueObj['種類'].endsWith('強化')) {
                if (!detailObj['種類'].startsWith(valueObj['種類'].replace(/強化$/, ''))) {
                    return;
                }
            }
    ...
    let flatDmgStatusName = detailObj['種類'] + 'アップ';
    if (detailObj['ダメージバフ'] && !detailObj['ダメージバフ'].startsWith(detailObj['種類'].substring(0, 5))) {
        flatDmgStatusName = detailObj['ダメージバフ'].replace(/バフ$/, 'アップ');
    }
    if (statusObj[flatDmgStatusName] > 0) {
        if (DAMAGE_CATEGORY_ARRAY.includes(detailObj['種類'])) {
            let myResultWork = calculateDamageFromDetailSub(statusObj, statusObj[flatDmgStatusName], myバフArr, is会心Calc, is防御補正Calc, is耐性補正Calc, my元素, my防御無視, 0);
            // 複数回HITするダメージについては、HIT数を乗算します
            if (myHIT数 > 1) {
                myResultWork[1] *= myHIT数;
                if (myResultWork[2] != null) {
