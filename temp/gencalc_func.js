...
function calculateStatusObj(statusObj) {
    ...
    if (validTeamConditionValueArr.length > 0) {
        ...
        チーム詳細ArrVar.forEach(detailObj => {
            ...
            if (detailObj['種類'].indexOf('元素付与') != -1) {
                if ('元素付与' in teamStatusObj && '元素付与強度' in teamStatusObj) {
                    if (teamStatusObj['元素付与強度'] > detailObj['数値']) {
                        return;
                    }
                }
                teamStatusObj['元素付与'] = detailObj['種類'].substring(0, 1);
                teamStatusObj['元素付与強度'] = detailObj['数値'];   // 命ノ星座:10, 元素スキル:30, 元素爆発:40, 不変:99
            } else if (detailObj['数値']) {
    ...
    const html = makeステータス変化Html(statusObj, miscStatusObj);
    $('#その他オプションステータス変化').html(html);
    ...
    // ダメージ元素
    if ('元素付与' in statusObj) {
        switch (選択中キャラクターデータVar['武器']) {
            case '片手剣':
            case '両手剣':
            case '長柄武器':
                通常攻撃_元素Var = statusObj['元素付与'];
                重撃_元素Var = statusObj['元素付与'];
                落下攻撃_元素Var = statusObj['元素付与'];
                break;
        }
    }
