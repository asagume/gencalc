...
export type TEREnergy = {
    category: string, // キャラクター, 武器, 聖遺物
    name: string, // キャラクター名, 武器名, 聖遺物名
    energies: number[], // 元素エネルギー
    unit: number, // 単位
}

export type TERParticle = {
    category: string, // キャラクター, 武器, 聖遺物
    name: string, // キャラクター名, 武器名, 聖遺物名
    element: string, // 元素 or 白
    num: number, // 個数
}
...
export function getEnergyByCharacter(
    ...
    let result: TEREnergy | undefined;
    if (energies.filter(e => e > 0).length) {
        result = {
            category: CHARACTER_ENERGY,
            name: character,
            energies: energies,
            unit: 0.1,
        }
