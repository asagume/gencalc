...
export function countN(character?: string, rotationList?: TActionItem[]) {
    return rotationList?.length ?
        rotationList.filter(rotation => (!character || rotation.member == character) && rotation.action.startsWith('N')).reduce((sum, rotation) => sum + Number(rotation.action.substring(1) ?? '1'), 0)
        : 1;
}
/** 重撃使用回数 */
...
export function getEnergyByCharacter(
    ...
    resultArr.forEach(entry => {
        ...
        for (let i = 0; herEnergies.length; i++) {
            energies[i] += otherEnergy;
        }
...
// 元素粒子数：
//   拾得ディレイ=-1:1秒あたりの元素粒子数
// 拾得ディレイ
//   0:使用者本人が拾得/1:直後のアクション実行者が拾得/2～:/-1:全員で分ける（出場時間で按分する）
// 継続時間、クールタイム：元素スキルの継続時間、クールタイム
