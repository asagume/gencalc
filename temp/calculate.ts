        ...
        if (damageDetailMyCharacter) {
            for (const category of ['通常攻撃', '重撃', '落下攻撃']) {
                ...
                for (const talentDetail of talentDetailArr) {
                    if (talentDetail['条件'] && checkConditionMatches(talentDetail['条件'], validConditionValueArr, constellation, statsInput.statsObj) === 0) {
                        continue;
                    }
        ...
        for (let i = 0; i < validConditionValueArr.length; i++) {
            if (validConditionValueArr[i].startsWith(conditionKey + '@')) {
                ...
                if (reRet) {
                    ...
                    if (reRet[1]) tempVal = tempVal.replace('^' + reRet[1], '');
                    if (reRet[3]) tempVal = tempVal.replace(reRet[3] + '$', '');
                    return Number(reRet[2]);    // マッチ 倍率
                }
            }
        }
    } else if (opkind === '=') {
    ...
    try {
        ...
        if (detailObj['除外条件'] || detailObj['適用条件']) {
            ...
            [characterInput.damageDetailMyCharacter, characterInput.damageDetailMyWeapon, characterInput.damageDetailMyArtifactSets].forEach(damageDetail => {
                ...
                damageDetail.ステータス変更系詳細.filter(s => s.条件).forEach(damageDetailObj => {
                    ...
                    const matchesBefore = checkConditionMatches(conditionStr, validConditionValueArr, constellation, statsObj);
                    const matchesAfter = checkConditionMatches(conditionStr, validConditionValueArrAfter, constellation, statsObj);
        ...
        if (talentChangeDetailObjArr) {
            ...
            talentChangeDetailObjArr.forEach(valueObj => {
                let number = null;
                if (valueObj.条件) {
                    number = checkConditionMatches(valueObj.条件, validConditionValueArr, constellation, statsObj);
                    if (number === 0) {
                        return;
                    }
                }
