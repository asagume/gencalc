...
export async function calculateSupporter(
    ...
    const teamOptionDetail1: TDamageDetailObj[] = [];
    const teamOptionDetail2: TDamageDetailObj[] = [];
    const teamConditions = new Map<string, string[]>;
    ...
    if (characterInput.characterMaster['チームバフ']) {
        const detailObjArr = characterInput.characterMaster.チームバフ as any[];
        ...
        damageDetailObjArr.forEach((damageDetailObj) => {
            const condition = makeSupporterCondition(character, damageDetailObj.名前, damageDetailObj.条件);
            damageDetailObj.条件 = condition;
            const changeKind = getChangeKind(damageDetailObj.種類 as string);
            if (changeKind === 'STATUS' &&
                teamOptionDetail1.filter((s) => s.条件 == condition && s.種類 == damageDetailObj.種類).length === 0
            ) {
                teamOptionDetail1.splice(teamOptionDetail1.length, 0, damageDetailObj);
            }
            if (changeKind === 'TALENT' &&
                teamOptionDetail2.filter((s) => s.条件 == condition && s.種類 == damageDetailObj.種類).length === 0
            ) {
                teamOptionDetail2.splice(teamOptionDetail2.length, 0, damageDetailObj);
            }
    ...
    optionInput.supporters[supporter] = {
        ...
        teamOptionDetail1: teamOptionDetail1,
        teamOptionDetail2: teamOptionDetail2,
