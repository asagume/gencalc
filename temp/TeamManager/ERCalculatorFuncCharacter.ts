...
import { getParticleInfo, getParticleNumFromInfo } from "@/particlemaster";
import { TActionItem, TTeamMemberResult, NUMBER_OF_MEMBERS, getCharacterMaster, getCharacterDetail } from "./team";
import { countQ, RECHARGE_ENERGY_CONSTELLATION, RECHARGE_ENERGY_SKILL, RECHARGE_ENERGY_BURST, countE, countC, RECHARGE_ENERGY_PASSIVE, countN, getCharacterInputValue, getStatsInputValue, RECHARGE_PARTICLE_CONSTELLATION, RECHARGE_PARTICLE_PASSIVE } from "./ERCalculatorCommon";
...
export const CHARACTER_ENERGY_FUNC: {
    ...
    '旅人(雷)': (character, constellation, members, rotationLength, rotationList, teamMemberResult) => { // eslint-disable-line
        ...
        const skillLevel = getCharacterInputValue(character, '元素スキルレベル', teamMemberResult);
        const er = getStatsInputValue(character, '元素チャージ効率', teamMemberResult);
        ...
        const burstLevel = getCharacterInputValue(character, '元素爆発レベル', teamMemberResult);
    ...
    'ウェンティ': (character, constellation, members, rotationLength, rotationList, teamMemberResult) => { // eslint-disable-line
        ...
        for (const element of ['炎', '水', '雷', '氷']) {
            ...
            for (let herIndex = 0; herIndex < members.length; herIndex++) {
                const herCm = getCharacterMaster(members[herIndex]);
                if (herCm?.元素 == element) {
    ...
    '雷電将軍': (character, constellation, members, rotationLength, rotationList, teamMemberResult) => { // eslint-disable-line
        ...
        if (teamMemberResult && teamMemberResult[character]) {
            teamMemberResult[character].damageResult.元素爆発.forEach(entry => {
        ...
        const myEnergy = 0;
        const allEnergy = unit * 5 * countQ(character, rotationList);
        const otherEnergy = 0;
        const herEnergies = _.fill(Array(NUMBER_OF_MEMBERS), 0);
        result.push([RECHARGE_ENERGY_BURST, '', myEnergy, allEnergy, otherEnergy, herEnergies, messages]);
        return result;
    ...
    '荒瀧一斗': (character, constellation, members, rotationLength, rotationList, teamMemberResult) => { // eslint-disable-line
        ...
        if (constellation >= constellationLevel) {
            ...
            const geoCount = members.filter(member => getCharacterMaster(member)?.元素 === '岩').length;
    ...
    '放浪者': (character, constellation, members, rotationLength, rotationList, teamMemberResult) => { // eslint-disable-line
        ...
        const electroCount = members.filter(member => getCharacterDetail(member)?.元素 === '雷').length;
...
export const CHARACTER_PARTICLE_EXTRA_FUNC: {
    ...
    'フィッシュル': (character, constellation, members, rotationLength, rotationList) => { // eslint-disable-line
        ...
        if (constellation >= 6) {
            ...
            const particleInfo = getParticleInfo(character, 'E', 6);
            const myParticle = 0;
            const allParticle = particleInfo ? getParticleNumFromInfo(particleInfo) * countN(undefined, rotationList) : 1;
