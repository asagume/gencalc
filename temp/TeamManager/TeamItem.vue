      ...
      <table>
        <tr>
          <td v-for="member in team.members" :key="member.id">
            <MemberItem :member="member" :statObj="memberStats(member.name)" :displayStat="displayStat"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const memberResultMap: Map<string, TMemberResult> = new Map();
    ...
    watch(props, async (newVal) => {
      ...
      for (const member of newVal.team.members) {
        ...
        memberResultMap.set(member.name, temp);
      }
      watchCount.value++;
    });
    ...
    const memberStats = (name: string) => {
      watchCount.value;
      let result: TStats = {};
      if (name) {
        const memberResult = memberResultMap.get(name);
        if (memberResult) {
          const statsInput = memberResult.statsInput;
          result = statsInput.statsObj;
        }
      }
      return result;
    };
