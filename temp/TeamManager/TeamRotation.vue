      ...
      <draggable :list="rotationList" item-key="id" :sort="true" handle=".handle">
        <template #item="{ element }">
          <div class="rotation-item">
            ...
            <div :class="'action-item with-tooltip' + colorClass(element.member)">
  ...
  setup(props) {
    ...
    function getElementalSkillCode(name: string) {
      ...
      if (master && master?.元素スキル?.説明) {
        ...
        if (description.indexOf('>一回押し<') !== -1) {
          result.push('E.Press');
        }
        if (description.indexOf('>長押し<') !== -1) {
          result.push('E.Hold');
        }
        if (result.length == 0) {
          result.push('E');
