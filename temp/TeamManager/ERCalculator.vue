                ...
                <dd>
                    <ol>
                        ...
                        <li>フィッシュル、鍾離、アルベド、珊瑚宮心海、八重神子 等<br>
                            設置物からの攻撃命中時に元素粒子が発生するタイプの元素スキルは、
...
import { getCharacterDetail, getCharacterMaster, getWeaponMaster, setupCharacterDetailMap, TActionItem, TConstellation, TTeam, TTeamMemberResult } from "./team";
...
export default defineComponent({
    ...
    setup(props) {
        ...
        watch(() => _.cloneDeep(props), (newVal, oldVal) => {
            if (!_.isEqual(newVal.team, oldVal.team)) {
        ...
        function getConstellation(character: string, team: TTeam, teamMemberResult?: TTeamMemberResult) {
            if (teamMemberResult) {
                return teamMemberResult[team.members.map(member => member.name).indexOf(character)]?.characterInput.命ノ星座 ?? (props.constellations ? props.constellations[character] ?? 0 : 0);
            } else {
                return props.constellations ? props.constellations[character] ?? 0 : 0;
            }
        }

        function updateRotationLength() {
            const memberNameArr = props.team.members.map(member => member.name);
            const eLength = [0, 0, 0, 0];
            const qLength = [0, 0, 0, 0];
            if (props.rotationList.length) {
                for (let i = 0; i < props.rotationList.length; i++) {
                    const rotation = props.rotationList[i];
            ...
            const newLength = Math.max(...eLength, ...qLength) ?? DEFAULT_ROTATION_LENGTH;
            if (rotationLength.value != newLength) {
        ...
        const setupInput = (team: TTeam, rotationList: TActionItem[], teamMemberResult?: TTeamMemberResult) => {
            ...
            const onFields = getOnFieldRate(team, rotationLength.value, rotationList, constellations);
            for (let i = 0; i < team.members.length; i++) {
                ...
                newInput.push({
                    ...
                    energyCost: characterDetail?.元素爆発.元素エネルギー ?? 40,
