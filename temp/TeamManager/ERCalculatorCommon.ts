import { CHARACTER_INPUT_TEMPLATE, STATS_INPUT_TEMPLATE } from "@/input";
import { CHARACTER_E_UNTIL_MAP, getParticleInfo, getDurationFromInfo, getReceiveTypeFromInfo, SP_SELF } from "@/particlemaster";
import _ from "lodash";
import { TActionItem, TTeam, NUMBER_OF_MEMBERS, getCharacterMaster, TTeamMemberResult } from "./team";
...
export function getCharacterInputValue(character: string, item: string, teamMemberResult?: TTeamMemberResult) {
    let characterInput = CHARACTER_INPUT_TEMPLATE;
    if (teamMemberResult) {
        characterInput = teamMemberResult[character].characterInput;
    }
    return (characterInput as any)[item];
}

export function getStatsInputValue(character: string, item: string, teamMemberResult?: TTeamMemberResult) {
    let statsInput = STATS_INPUT_TEMPLATE;
    if (teamMemberResult) {
        statsInput = teamMemberResult[character].statsInput;
    }
    return (statsInput.statsObj as any)[item];
}
