...
async function setupTeamOption(optionInput) {
    ...
    Object.keys(チームオプションMasterVar).forEach(optionKey => {
        ...
        if (!teamOptionConditionMap.has(character)) {
            teamOptionConditionMap.set(character, new Map());
        }
        const teamOptionCondition = teamOptionConditionMap.get(character);
        if (damageDetailObj['条件'].size > 0) {
            damageDetailObj['条件'].forEach((value, key) => {
                ...
                teamOptionCondition.set(key, value);
            });
        } else {
            teamOptionCondition.set(optionKey, null);
        }
    });
    return optionInput;
}
