 ...
 * @returns {object}
 */
function calculateStatus(characterInput, statusInput, artifactDetailInput) {
    ...
    Object.keys(characterMaster['ステータス']).forEach(stat => {
        ['基礎ステータス'].forEach(category => {
            let toStat = ['HP', '攻撃力', '防御力'].includes(stat) ? stat + '+' : stat;
            if (toStat in result[category]) {
                result[category][toStat] += getStatValueByLevel(characterMaster['ステータス'][stat], characterInput['突破レベル'], characterInput['レベル']);
            }
        });
        ['高級ステータス', 'その他'].forEach(category => {
            let toStat = ['HP', '攻撃力', '防御力'].includes(stat) ? stat + '+' : stat;
            if (toStat in result[category]) {
                result[category][toStat] += getPropertyValueByLevel(characterMaster['ステータス'][stat], characterInput['突破レベル']);
            }
        });
    });
    ...
    Object.keys(weaponMaster['ステータス']).forEach(stat => {
        ['基礎ステータス', '高級ステータス', 'その他'].forEach(category => {
            let toStat = ['HP', '攻撃力', '防御力'].includes(stat) ? stat + '+' : stat;
            if (toStat in result[category]) {
                result[category][toStat] += getStatValueByLevel(weaponMaster['ステータス'][stat], characterInput['武器突破レベル'], characterInput['レベル']);
            }
        });
    });
        ...
        Object.keys(artifactDetailInput['聖遺物ステータス']).forEach(stat => {
            ['高級ステータス', 'その他'].forEach(category => {
                if (stat in result[category]) {
                    result[category][stat] += artifactDetailInput['聖遺物ステータス'][stat];
                }
            });
        });
    }
