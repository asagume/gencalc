...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const setupSupporterDamageResult = (savedSupporter: {
      ...
      // 聖遺物4セット効果のチームバフを取り込みます
      let artifactSet4: TArtifactSetKey | undefined = undefined;
      if (characterInput.artifactSets[0] && characterInput.artifactSets[1] && characterInput.artifactSets[0] == characterInput.artifactSets[1]) {
        artifactSet4 = characterInput.artifactSets[0];
      }
      const artifactSet4Options: any[] = [];
      if (artifactSet4) {
        const masterArr = ARTIFACT_SET_MASTER_LIST.filter(master => master.key == artifactSet4);
        if (masterArr.length) {
          const master = masterArr[0];
          if (master['4セット効果']) {
            if (master['4セット効果']['詳細']) {
              master['4セット効果']['詳細'].forEach(detailObj => {
                if (detailObj['チーム']) {
                  artifactSet4Options.push(detailObj);
                }
              })
            }
          }
      ...
      if (artifactSet4Options.length) {
        takeMasterTeamOption(characterInput.character, {
          チームバフ: artifactSet4Options,
        });
      }
