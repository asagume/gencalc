...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const teamOptionDetails1 = computed(() => {
      const result: TDamageDetailObj[] = [];
      Object.keys(supporters).forEach(key => {
        const work = supporters[key]?.teamOptionDetails1;
        if (work) {
          result.push(...work);
        }
      })
    ...
    const teamOptionDetails2 = computed(() => {
      const result: TDamageDetailObj[] = [];
      Object.keys(supporters).forEach(key => {
        const work = supporters[key]?.teamOptionDetails2;
        if (work) {
          result.push(...work);
        }
      })
    ...
    const conditionMap = computed(() => {
      const result = new Map<string, string[] | object | null>();
      Object.keys(supporters).forEach(key => {
        const work = supporters[key]?.teamConditions;
        if (work) {
          work.forEach((value2, key2) => {
    ...
    const exclusionMap = computed(() => {
      ...
      Object.keys(supporters).forEach(key => {
        const work = supporters[key]?.teamExclusions;
        if (work) {
          work.forEach((value2, key2) => {
            result.set(key2, value2);
          })
    ...
    const artifactSetIconSrc = (character: string, index: number) => {
      const work = supporters[character];
      return _.isArray(work?.characterInput?.artifactSetMasters) ? work.characterInput.artifactSetMasters[index].icon_url ?? IMG_SRC_DUMMY : IMG_SRC_DUMMY;
    }
    const artifactSetName = (character: string, index: number) => {
      const work = supporters[character];
      return _.isArray(work?.characterInput?.artifactSetMasters) ? work.characterInput.artifactSetMasters[index].key ?? '' : '';
    }
    ...
    const initializeValues = (input: TConditionInput) => {
      Object.keys(conditionValues).forEach((key) => {
        if (key in input.conditionValues) {
