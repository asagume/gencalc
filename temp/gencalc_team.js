...
const setupDamageDetailDataCharacter4Team = function (damageDetailObj, changeDetailObj, inputObj, characterMasterObj) {
...
// 武器データより
const setupDamageDetailDataWeapon4Team = function (changeDetailObj, inputObj, weaponMasterObj) {
...
// 聖遺物セットデータより
const setupDamageDetailDataArtifactSet4Team = function (changeDetailObj, inputObj) {
...
function calculateStatus4Team(statusObj, characterMasterObj, kind, formulaArr, opt_max = null) {
    ...
    console.debug(calculateStatus4Team.name, null, kind, formulaArr, '=>', result);
}

function setupStatus4Team(statusObj, inputObj, characterMasterObj, weaponMasterObj, changeDetailObj) {
    ...
    Object.keys(inputObj).filter(s => s.startsWith('聖遺物サブ効果')).forEach(inputKey => {
        ...
        if (key == 'HP') {
            toKey = 'HP上限';
        } else if (key.endsWith('P')) { // 攻撃力P, 防御力P
            tokey = key.replace(/P$/, '乗算');
        }
        statusObj[toKey] = inputObj[inputKey];
    });
    ...
    Object.keys(changeDetailObj).forEach(key => {
        ...
        changeDetailObj[key]['ステータス'].forEach(detailObj => {
            ...
            if (detailObj['条件']) {
                let number = checkConditionMatches(statusObj, detailObj['条件'], validConditionValueArr);
                if (number == 0) return;
                if (number != 1) {
            ...
            if (myPriority1KindArr.includes(detailObj['種類'])) { // 攻撃力の計算で参照されるものを先に計上するため…
                myPriority1KindFormulaArr.push([detailObj['種類'], myNew数値, detailObj['最大値']]);
            } else if (detailObj['種類'].endsWith('%')) {  // 乗算系(%付き)のステータスアップを先回しします HP 攻撃力 防御力しかないはず
                myPriority2KindFormulaArr.push([detailObj['種類'], myNew数値, detailObj['最大値']]);
            } else {
                myKindFormulaArr.push([detailObj['種類'], myNew数値, detailObj['最大値']]);
            }
        });
    });
    ...
    myPriority1KindFormulaArr.forEach(entry => {
        calculateStatus(statusObj, characterMasterObj, entry[0], entry[1], entry[2]);
    });
...
// {上記}^{排他条件名}
function checkConditionMatchesSub4Team(statusObj, conditionStr, validConditionValueArr) {
...
const checkConditionMatches4Team = function (statusObj, conditionStr, validConditionValueArr) {
    ...
    if (myCondStr.indexOf('|') != -1) {  // |はOR条件です
        ...
        for (let i = 0; i < myCondStrArr.length; i++) {
            let resultSub = checkConditionMatchesSub4Team(statusObj, myCondStrArr[i], validConditionValueArr);
            if (resultSub == 1) {
                return 1;   // マッチ
            }
        }
...
function calculateDamageFromDetail(statusObj, detailObj, changeDetailObj, validConditionValueArr, opt_element = null) {
    ...
    Object.keys(changeDetailObj).forEach(key => {
        ...
        changeDetailObj[key]['天賦性能'].forEach(entry => {
            let number = null;
            if (entry['条件']) {
                number = checkConditionMatches4Team(statusObj, entry['条件'], validConditionValueArr);
                if (number == 0) {
                    return;
                }
            }
            ...
            if (entry['種類'].endsWith('元素付与')) {   // 元素付与は先んじて適用します
                ...
                let myValue = calculateFormulaArray(statusObj, entry['数値'], entry['最大値']);
                my防御無視 += myValue;
...
function setupDamageResult4Team(inputObj, damageDetailObj, changeDetailObj, statusObj) {
    statusObj['ダメージ計算'] = {};
    ...
    damageDetailObj['元素スキル'].forEach(detailObj => {
        ...
        statusObj['ダメージ計算']['元素スキル'].push(calculateDamageFromDetail(statusObj, detailObj, changeDetailObj, validConditionValueArr, null));
    });
    ...
    damageDetailObj['元素爆発'].forEach(detailObj => {
        ...
        statusObj['ダメージ計算']['元素爆発'].push(calculateDamageFromDetail(statusObj, detailObj, changeDetailObj, validConditionValueArr, null));
    });
...
async function teamTest() {
    ...
    inputObj['武器レベル'] = '90';
    inputObj['精錬ランク'] = '5';
    inputObj['聖遺物セット効果1'] = '旧貴族のしつけ';
    inputObj['聖遺物セット効果2'] = '旧貴族のしつけ';
    inputObj['聖遺物メイン効果1'] = '5_HP';
    inputObj['聖遺物メイン効果2'] = '5_攻撃力';
    inputObj['聖遺物メイン効果3'] = '5_元素チャージ効率';
    inputObj['聖遺物メイン効果4'] = '5_炎元素ダメージバフ';
    inputObj['聖遺物メイン効果5'] = '5_会心率';
    ...
    setupDamageDetailDataCharacter4Team(damageDetailObj, changeDetailObj, inputObj, characterMasterObj);
    setupDamageDetailDataWeapon4Team(changeDetailObj, inputObj, weaponMasterObj);
    setupDamageDetailDataArtifactSet4Team(changeDetailObj, inputObj);

    let statusObj = JSON.parse(JSON.stringify(ステータス詳細ObjTemplate));

    statusObj['レベル'] = Number(inputObj['レベル'].replace('+', ''));

    setupStatus4Team(statusObj, inputObj, characterMasterObj, weaponMasterObj, changeDetailObj);

    console.log(statusObj);

    setupDamageResult4Team(statusObj, inputObj, damageDetailObj, changeDetailObj);
