...
function initialSetupCharacterOwnList() {
    const CharacterOwnList = {
        ...
        methods: {
            ...
            saveOnClick: function () {
                localStorage.setItem('キャラクター所持状況', JSON.stringify(this.命ノ星座));
                isChanged = false;
            }
        }
    }
    ...
    const WeaponOwnList = {
        ...
        methods: {
            ...
            onClick: function (item) {
                ...
                } else if (this.精錬ランク[item.name] > 0 && item.master['レアリティ'] <= 3) {
            ...
            saveOnClick: function () {
                localStorage.setItem('武器所持状況', JSON.stringify(this.精錬ランク));
                isChanged = false;
            }
        }
    }
