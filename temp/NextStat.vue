...
import { calculateDamageResult, calculateRotationTotalDamage, calculateStats, TRotationDamageEntry } from "@/calculate";
...
export default defineComponent({
    ...
    props: {
        ...
        rotationDamageArr: { type: Array as PropType<TRotationDamageEntry[]> },
    },
    emits: ["update:stat-adjustments"],
    setup(props, context) {
        const { displayName, displayStatValue } = CompositionFunction();

        const ROTATION_TOTAL_DMG_NAME = 'ROTATION TOTAL DMG';
        ...
        function getDamageValue(itemName: string, damageResult: TDamageResult) {
            ...
            if (itemName == ROTATION_TOTAL_DMG_NAME) {
                if (props.rotationDamageArr) {
                    result = calculateRotationTotalDamage(props.rotationDamageArr, damageResult);
                }
            } else {
        ...
        watch(props, async (newVal, oldVal) => {
            const newCharacterInput = newVal.characterInput;
            if (newCharacterInput) {
                const oldCharacterInput = oldVal.characterInput;
                if (newCharacterInput.character != oldCharacterInput?.character) {
                    resetButtonOnClick();
                    return;
                }
            }
            const newRotationDamageArr = newVal.rotationDamageArr;
            if (newRotationDamageArr) {
                const oldRotationDamageArr = oldVal.rotationDamageArr;
                if (newRotationDamageArr.length != oldRotationDamageArr?.length) {
