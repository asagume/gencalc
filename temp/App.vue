...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(async () => {
      ...
      await Promise.all(CHARACTER_KEYS.map(key => calculateSupporter(defaultSupporterOptionInput, key as TCharacterKey)));
    ...
    async function _setupSupporters() {
      ...
      Object.keys(optionInputRea.supporters).forEach(key => {
        const teamConditions = optionInputRea.supporters[key]?.teamConditions;
        if (teamConditions === undefined) {
          return;
        }
      ...
      optionInputRea.teamOption.checkboxList.splice(0, optionInputRea.teamOption.checkboxList.length, ...checkboxList);
      optionInputRea.teamOption.selectList.splice(0, optionInputRea.teamOption.selectList.length, ...selectList);
      optionInputRea.teamOption.numberList.splice(0, optionInputRea.teamOption.numberList.length, ...numberList);
