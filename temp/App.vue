  ...
  <div class="base-container">
    ...
    <div class="pane3">
      <CharacterInput :characterInput="characterInputRef" :recommendationList="recommendationList"
        ...
        @update-character-input="updateCharacterInput($event)" />
    </div>
    <div class="pane4">
    ...
    <div class="pane6">
      ...
      <div v-if="pane6Toggle2Ref" style="margin-bottom: 10px">
        ...
        <template v-if="statInputTabRef == 1">
          <StatsInput :statsObj="statsObjRef" :categoryList="characterStats1CategoryList" />
        </template>
        <template v-if="statInputTabRef == 2">
          <StatsInput :statsObj="statsObjRef" :categoryList="characterStats2CategoryList" />
        </template>
        <template v-if="statInputTabRef == 3">
          ...
          <StatsInput :statsObj="statsObjRef" :categoryList="enemyStatsCategoryList" />
        </template>
      </div>
      <div v-if="pane6Toggle3Ref" style="margin-bottom: 10px">バフ/デバフ</div>
    </div>
...
export default defineComponent({
  ...
  props: {
    ...
    artifactDetailInput: { type: Object, require: true },
  ...
  setup(props) {
    ...
    const artifactDetailInputRef = ref(props.artifactDetailInput);
    const conditionInputRef = ref(props.conditionInput);

    const characterSelectVisibleRef = ref(false);
    const character = computed(() =>
      characterInputRef.value ? characterInputRef.value.character : undefined
    );
    ...
    const artifactSets = reactive(characterInputRef.value?.artifactSets ?? ["NONE", "NONE"]);
    const artifactDetailInputVisibleRef = ref(false);

    const statsInput = reactive(JSON.parse(JSON.stringify(STATS_INPUT_TEMPLATE)));
    ...
    const updateRecommendation = async (recommendation: TRecommendation) => {
      if (!characterInputRef.value || !artifactDetailInputRef.value || !conditionInputRef.value)
        return;
      ...
      makeDamageDetailObjArrObjCharacter(characterInputRef.value, conditionInputRef.value); // キャラクター
      makeDamageDetailObjArrObjWeapon(characterInputRef.value, conditionInputRef.value); // 武器
      makeDamageDetailObjArrObjArtifactSets(characterInputRef.value, conditionInputRef.value); // 聖遺物セット効果

      calculateStats(statsInput, characterInputRef.value as any, artifactDetailInputRef.value as any, conditionInputRef.value as any);
    ...
    // 
    const updateCharacterInput = (characterInput: any) => {
      Object.keys(characterInput).forEach(key => {
        if (characterInputRef.value && key in characterInputRef.value) {
          [key] = characterInput[key];
        }
      });
      const inputKeys = Object.keys(characterInput);
      if (inputKeys.filter(s => ['突破レベル', 'レベル', '命ノ星座', '通常攻撃レベル', '元素スキルレベル', '元素爆発レベル'].includes(s)).length > 0) {
        makeDamageDetailObjArrObjCharacter(characterInputRef.value, conditionInputRef.value); // キャラクター
      }
      if (inputKeys.filter(s => ['武器突破レベル', '武器レベル', '武器精錬ランク'].includes(s)).length > 0) {
        makeDamageDetailObjArrObjWeapon(characterInputRef.value, conditionInputRef.value); // 武器
      }
      calculateStats(statsInput, characterInputRef.value as any, artifactDetailInputRef.value as any, conditionInputRef.value as any);
      console.log(statsInput.statsObj);
