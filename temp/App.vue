    ...
    <div class="pane6">
      ...
      <template v-if="pane6Toggle2">
        ...
        <template v-if="statInputTab == 1">
          <Stats1Input :statusObj="statusObj" />
        </template>
...
import Stats1Input from './components/Stats1Input.vue';
...
export default defineComponent({
  ...
  components: {
    CharacterSelect, CharacterInput, WeaponSelect, ArtifactSetSelect, ArtifactDetailInput, Stats1Input,
  },
  setup(props) {
    ...
    const statusObj = ref(JSON.parse(JSON.stringify(ステータスTEMPLATE)));
    ...
    const updateCharacter = async function (character: TCharacterKey) {
      ...
      Object.keys(ステータスTEMPLATE).forEach(key => {
        statusObj[key] = ステータスTEMPLATE[key];
      });
    };
    // 武器選択画面を開きます/閉じます
