...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const mainstats = reactive(props.artifactDetailInput?.聖遺物メイン効果 ?? [null, null, null]);
    const prioritySubstats = reactive(props.artifactDetailInput?.聖遺物優先するサブ効果 ?? [null, null, null]);
    const prioritySubstatIndices = reactive(props.artifactDetailInput?.聖遺物優先するサブ効果上昇値 ?? [-1, -1, -1]);
    const prioritySubstatCounts = reactive(props.artifactDetailInput?.聖遺物優先するサブ効果上昇回数 ?? [0, 0, 0]);
    ...
    const substatMin = (substat: string) => substat in artifactStatsMain ? artifactStatsMain[substat] : 0;
    const substatStep = (substat: string) => ['HP', '攻撃力', '防御力', '元素熟知'].includes(substat) ? 1 : 0.1;
    ...
    const _calculateArtifactStatsPrioritySub = () => {
      ...
      const prioritySubstatValues = [prioritySubstatValueList(0), prioritySubstatValueList(1), prioritySubstatValueList(2)];
      calculateArtifactSubStatByPriority(artifactStatsSub, mainstats, prioritySubstats, prioritySubstatIndices, prioritySubstatValues, prioritySubstatCounts);
    };
    /** 聖遺物ステータスを計算します */
    ...
    const updateMainstats = () => {
      _calculateArtifactStatsMain();
      calculateArtifactStats();
      context.emit("update:artifact-detail", artifactDetailInputRef.value);
    };
    /** 優先するサブ効果が更新されました */
